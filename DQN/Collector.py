...
class Collector(object):
    ...
    def __init__(
        ...
        take_place_of_batch: bool = False
    ) -> None:
        super().__init__()
        ...
        self.take_place_of_batch = take_place_of_batch
    ...
    def get_buffer_to_optim(self, amount):
        observations = self.buffer.__getattr__('obs')
        rewards = self.buffer.__getattr__('rew')
        # print('first two invalid rewards:{}, {}'.format(rewards[self.buffer._size], rewards[self.buffer._size+1]))
        rewards = rewards[:self.buffer._size]
        # print('last two valid rewards:{}, {}'.format(rewards[-2], rewards[-1]))
        abs_rewards = np.absolute(rewards)
        index = np.argpartition(abs_rewards, amount)[:amount]
        # print(abs_rewards[index])
        return observations, rewards, index
    ...
    def collect(
        ...
        # print('step count:{}'.format(self.collect_step))
        # print(self.collect_step)
