    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def update(self, sample_size: int, buffer: Optional[ReplayBuffer],
        ...
        if np.random.uniform() < epsilon:
            observations, rewards, index = buffer.sample_low_reward_indices(sample_size)
            for idx in index:
                new_batch = Batch(
                    obs={}, act={}, rew={}, done={}, obs_next={}, info={}, policy={}
                )
                new_batch.update(obs=[observations[idx]])
                i = 0
                while i<10 :
                    with torch.no_grad():  # faster than retain_grad version
                        # self.data.obs will be used by agent to get result
                        result = self.forward(new_batch, None)
                    ...
                    act = self.exploration_noise(act, new_batch)
                    new_batch.update(act=act)
                    # get bounded and remapped actions first (not saved into buffer)
                    action_remap = self.map_action(new_batch.act)
                    # step in env

                    result = self.env.step(action_remap, self.ready_env_ids)  # type: ignore
                    obs_next, rew, done, info = result
                    # print('new reward:{}'.format(rew))
                    i += 1
                    if np.abs(rew) > np.abs(rewards[idx]):
                        break
                if np.abs(rew) > np.abs(rewards[idx]):
                    # new_batch.update(act=act, rew=rew, done=done, obs_next=obs_next, info=info,
                    #           policy=policy)
                    buffer.replace_transaction(idx, obs=observations[idx], act=act, rew=rew, done=done, obs_next=obs_next, info=info,
                              policy=policy)
            batch, indices = buffer[index], index
        else:
